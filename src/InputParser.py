...
class InputParser:
        ...
        if re.compile("[a-h][1-8]?[qrbn]?").match(shortNotation):
            for move in moves:
                if (
                    ...
                    and self.board.getCoordinateNotationOfMove(move)
                    .replace("=", "")
                    .lower()
            ...
            for move in moves:
                if (
                    ...
                    and not move.pieceToCapture
                    and re.sub(
                        "[1-8]", "", self.board.getCoordinateNotationOfMove(move)
                    )
                    .replace("=", "")
                    .lower()
                    .endswith(shortNotation)
    ...
    def getLegalMovesWithNotation(self, side, short=True):
        ...
        for duplicateMove in duplicateNotationMoves:
            duplicateMove.notation = (
                self.board.getAlgebraicNotationOfMoveWithFileAndRank(
                    duplicateMove, short
                )
            )
    ...
    def duplicateMovesFromMoves(self, moves):
        return list(
            filter(
                lambda move: len([m for m in moves if m.notation == move.notation]) > 1,
                moves,
            )
        )
